...
summary(lm1)
anova(lm1)
#linear regression of weight regressed on treatment and sex, with both linear model summary and ANOVA output summary.  Treatment is treated as a factor in this analysis (4 factors, each an individual treatment)
...
aov.out.weight <- aov(weight ~ SEA + stress + Sex + Sex + SEA*stress*day + Error(animal_id/day), data = animal_weight_outlier)
summary(aov.out.weight)

#summary(lme(weight ~ SEA + stress + Sex + SEA*stress*day, data = animal_weight_outlier, random = ~ 1 | treatment))
